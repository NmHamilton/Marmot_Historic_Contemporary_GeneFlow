...
REALIGNED_BAMS=/path-to-working-directory/snp-calling/*realigned.bam
...
#create a variable with the sample name using the name of the dedup bam file. We use the cut command, using the character '/' as field delimiter.     
    OUTPUT_BASENAME=$(echo $sample | cut -d/ -f4)
...
java -Xmx4g -jar /user/GenomeAnalysisTK-3.46/GenomeAnalysisTK.jar \
-T SelectVariants \
-R $REFERENCE  \
...
#taxon or sample we are working now
    echo "Processing $sample"
#create a variable with the sample name using the name of the dedup bam file. We use the cut command, using the character '/' as field delimiter.     
    FILE_BASENAME=$(echo $sample | cut -d/ -f4)
    echo $FILE_BASENAME
...
#execute the command in GATK for base recalibration
   ...
   eval $(echo "java -Xmx4g -jar /user/GenomeAnalysisTK-3.46/GenomeAnalysisTK.jar -T BaseRecalibrator -R $REFERENCE -I $sample -knownSites genotyped_X_samples_only_PASS_snp.vcf -o $TABLE_NAME")  
   eval $(echo "java -Xmx4g -jar /user/GenomeAnalysisTK-3.46/GenomeAnalysisTK.jar -T PrintReads -R $REFERENCE -I $sample -BQSR $TABLE_NAME -o $RECAL_OUT")

    #mv $RECAL_OUT ${RECAL_OUT//All_dedup_realigned_recal/recal}
    #mv $RECAL_OUT_bai ${RECAL_OUT_bai//All_dedup_realigned_recal/recal}
done

RECAL_BAMS=/path-to-working-directory/snp-calling/*_recal.bam
...
#create a variable with the sample name using the name of the recalibrated bam file. We use the cut command, using the character '/' as field delimiter.      
    RECAL1_BASENAME=$(echo $bam_recal | cut -d/ -f4)
...
# taxon or sample we are working now
    echo "Processing $sample"
# create a variable with the sample name using the name of the original bam file. We use the cut command, using the character '/' as field delimiter.     
    FILE2_BASENAME=$(echo $sample | cut -d/ -f4)
    echo $FILE2_BASENAME
...
# execute the command in GATK for base recalibration
   ...
   eval $(echo "java -Xmx4g -jar /user/GenomeAnalysisTK-3.46/GenomeAnalysisTK.jar -T BaseRecalibrator -R $REFERENCE -I $sample -knownSites genotyped_X_samples_only_PASS_snp_2nd.vcf -o $TABLE2_NAME")  
   eval $(echo "java -Xmx4g -jar /user/GenomeAnalysisTK-3.46/GenomeAnalysisTK.jar -T PrintReads -R $REFERENCE -I $sample -BQSR $TABLE2_NAME -o $RECAL2_OUT")
...
RECAL2_BAMS=/path-to-working-directory/snp-calling/*_2recal.bam
...
# create a variable with the sample name using the name of the recalibrated bam file. We use the cut command, using the character '/' as field delimiter.      
    RECAL2_BASENAME=$(echo $bam2_recal | cut -d/ -f4)
...
#create a variable with the sample name using the name of the 2nd recalibrated  bam file. We use the cut command, using the character '/' as field delimiter. In my case, this will cut the taxon path into 9 fields. The command -f8 tells to keep only the 9th field, which is the sample name     
    FILE3_BASENAME=$(echo $sample | cut -d/ -f4)
    echo $FILE3_BASENAME
...
#execute the command in GATK for base recalibration
   ...
   eval $(echo "java -Xmx4g -jar /user/GenomeAnalysisTK-3.46/GenomeAnalysisTK.jar -T BaseRecalibrator -R $REFERENCE -I $sample -knownSites genotyped_X_samples_only_PASS_snp_3rd.vcf -o $TABLE3_NAME")  
   eval $(echo "java -Xmx4g -jar /user/GenomeAnalysisTK-3.46/GenomeAnalysisTK.jar -T PrintReads -R $REFERENCE -I $sample -BQSR $TABLE3_NAME -o $RECAL3_OUT")
...
RECAL3_BAMS=/path-to-working-directory/snp-calling/*_3recal.bam
...
# create a variable with the sample name using the name of the recalibrated bam file. We use the cut command, using the character '/' as field delimiter.     
    RECAL3_BASENAME=$(echo $bam3_recal | cut -d/ -f4)
...
#create a variable with the sample name using the name of the original  bam file. We use the cut command, using the character '/' as field delimiter. In my case, this will cut the taxon path into 9 fields. The command -f8 tells to keep only the 9th field, which is the sample name     
    FILE4_BASENAME=$(echo $sample | cut -d/ -f4)
    echo $FILE4_BASENAME
...
#execute the command in GATK for base recalibration
   ...
   eval $(echo "java -Xmx4g -jar /user/GenomeAnalysisTK-3.46/GenomeAnalysisTK.jar -T BaseRecalibrator -R $REFERENCE -I $sample -knownSites genotyped_X_samples_only_PASS_snp_4th.vcf -o $TABLE4_NAME")  
   eval $(echo "java -Xmx4g -jar /user/GenomeAnalysisTK-3.46/GenomeAnalysisTK.jar -T PrintReads -R $REFERENCE -I $sample -BQSR $TABLE4_NAME -o $RECAL4_OUT")
...
RECAL4_BAMS=/path-to-working-directory/snp-calling/*_4recal.bam
...
# create a variable with the sample name using the name of the recalibrated bam file. We use the cut command, using the character '/' as field delimiter.      
    RECAL4_BASENAME=$(echo $bam4_recal | cut -d/ -f4)
